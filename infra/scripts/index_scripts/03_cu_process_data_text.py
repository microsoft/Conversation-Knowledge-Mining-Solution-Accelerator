...
def get_secrets_from_kv(kv_name, secret_name):
    try:
        ...
        print(f"Error retrieving secret '{secret_name}' from Key Vault '{kv_name}': {e}")
        raise
...
try:
    ...
    print(f"Failed to set up Azure DataLake: {e}")
    raise
...
try:
    ...
    print(f"Failed to set up Azure Search: {e}")
    raise
...
try:
    ...
    print(f"Failed to establish SQL Server connection: {e}")
    raise
...
try:
    ...
    print(f"Failed to initialize Content Understanding client: {e}")
    raise
...
try:
    ...
    print("Azure AI Foundry clients initialized.")
except Exception as e:
    print(f"Failed to initialize Azure AI Foundry clients: {e}")
    raise
...
for path in paths:
    try:
        ...
        print(f"Error accessing file {path.name}: {e}")
        continue
...
if docs:
    try:
        ...
        print(f"Error uploading final batch to Azure Search: {e}")
...
def bulk_import_json_to_table(json_file, table_name):
    try:
        ...
        print(f"Warning: Sample data file '{json_file}' not found. Skipping import.")
    except Exception as e:
        print(f"Error importing data from {json_file} to {table_name}: {e}")
        raise
...
try:
    ...
    print("Warning: 'sample_search_index_data.json' not found. Skipping sample index data upload.")
except Exception as e:
    print(f"Error uploading sample search index data: {e}")
...
try:
    ...
    print(f"Error preparing topic mining table: {e}")
    raise
...
def call_gpt4(topics_str1, client):
    try:
        ...
        print(f"Error calling GPT-4 for topic mining: {e}")
        raise
...
try:
    ...
    print(f"Error mining topics or inserting into database: {e}")
    raise
...
try:
    ...
    print(f"Error loading mined topics: {e}")
    raise
...
def get_mined_topic_mapping(input_text, list_of_topics):
    try:
        ...
        print(f"Error getting mined topic mapping: {e}")
        raise
...
try:
    ...
    for _, row in df_processed_data.iterrows():
        try:
            ...
            print(f"Error mapping topic for ConversationId {row['ConversationId']}: {e}")
            continue
    conn.commit()
    print("Processed data mapped to mined topics.")
except Exception as e:
    print(f"Error processing data mapping: {e}")
    raise
...
try:
    ...
    print(f"Error updating km_processed_data table: {e}")
    raise
...
try:
    ...
    for _, row in df.iterrows():
        try:
            ...
            print(f"Error processing key phrases for ConversationId {row['ConversationId']}: {e}")
            continue
    conn.commit()
    print("processed_data_key_phrases table updated.")
except Exception as e:
    print(f"Error updating processed_data_key_phrases table: {e}")
    raise
