...
enable_public_access() {
	...
	original_storage_public_access=$(az storage account show \
		...
		-o tsv 2>&1)
	if [[ "$original_storage_public_access" == *"ERROR"* ]]; then
		echo "✗ Failed to get Storage Account public access status"
		return 1
	fi

	if [ -z "$original_storage_public_access" ] || [ "$original_storage_public_access" = "null" ]; then
		original_storage_public_access="Enabled"
	fi
...
get_values_from_azd_env() {
	...
	if [ -z "$resourceGroupName" ] || [ -z "$storageAccountName" ] || [ -z "$fileSystem" ] || [ -z "$sqlServerName" ] || [ -z "$SqlDatabaseName" ] || [ -z "$apiAppPrincipleId" ] || [ -z "$apiAppName" ] || [ -z "$aiSearchName" ] || [ -z "$aif_resource_id" ] || [ -z "$usecase" ] || [ -z "$solutionName" ]; then
...
bash "$SCRIPT_DIR/run_create_index_scripts.sh" "$resourceGroupName" "$aiSearchName" "$searchEndpoint" "$sqlServerName" "$SqlDatabaseName" "$apiAppName" "$apiAppPrincipleId" "$storageAccountName" "$openaiEndpoint" "$deploymentModel" "$embeddingModel" "$cuEndpoint" "$cuApiVersion" "$aif_resource_id" "$aiAgentEndpoint" "$usecase" "$solutionName"
if [ $? -ne 0 ]; then
