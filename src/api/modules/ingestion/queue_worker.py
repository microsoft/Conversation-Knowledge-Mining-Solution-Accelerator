...
class QueueWorker:
    ...
    def _handle_extraction(self, message, payload: dict):
        """Download raw file from blob → Content Understanding → enqueue for enrichment.

        Every file type (pdf, docx, xlsx, csv, txt, json, images, audio) is processed
        through Content Understanding. There is no local fast-path — CU is the single
        source of truth for extracted content and CU-enriched fields.
        """
        ...
        try:
            ...
            # 1. Download the raw file from blob storage
            ...
            content = container.get_blob_client(blob_path).download_blob().readall()
            content_size = len(content)

            # 2. Extract with Content Understanding (SAS URL preferred, byte-upload fallback)
            cu_wait_sec = content_understanding_service.resolve_max_wait(content_size, settings.cu_poll_max_wait_sec)
            extracted = self._run_cu_extraction(file_id, filename, content, cu_wait_sec)

            # 3. Bail out if CU could not extract any content
            if not extracted.markdown.strip():
                error_msg = f"No text could be extracted from {filename} (CU returned empty content)"
                logger.warning(f"[extraction] {error_msg}")
                ingestion_service._update_file_status(file_id, "failed", error=error_msg)
                queue_service.delete(EXTRACTION_QUEUE, message)
                return

            # 4. Persist extracted content and enqueue for enrichment
            self._complete_extraction(container, message, file_id, filename, ext, extracted)

        except TimeoutError:
            # Retry once with the full configured cap (byte upload = deterministic path).
            retry_wait_sec = settings.cu_poll_max_wait_sec
            if cu_wait_sec and retry_wait_sec > cu_wait_sec:
                logger.warning(
                    f"[extraction] CU timeout for {filename} at {cu_wait_sec}s; retrying once with {retry_wait_sec}s"
                )
                try:
                    ...
                    logger.error(f"[extraction] CU retry failed for {filename}: {e}")
            ...
            # Raw blob is missing (deleted/moved/mismatched path). Terminal for this message.
            ...
            error_msg = f"Invalid file format or parsing error: {e}"
            logger.error(f"[extraction] {error_msg}")
            ingestion_service._update_file_status(file_id, "failed", error=error_msg)
            queue_service.delete(EXTRACTION_QUEUE, message)
            ...
            ingestion_service._update_file_status(
                file_id, "failed",
                error=f"{error_msg} (See logs for details. File: {filename})",
            )
            queue_service.delete(EXTRACTION_QUEUE, message)
    ...
    def _run_cu_extraction(self, file_id: str, filename: str, content: bytes, max_wait_sec: int):
        """Run Content Understanding on a file, preferring a SAS URL over re-uploading bytes.

        The byte-upload path is also the deterministic path used for timeout retries.
        """
        from src.api.modules.ingestion.azure_storage import azure_storage_service
        from src.api.modules.document_intelligence.service import content_understanding_service
        ...
        if settings.cu_use_sas_url:
            try:
                ...
                if sas_url:
                    return content_understanding_service.analyze_url(
                        file_url=sas_url,
                        filename=filename,
                        max_wait_sec=max_wait_sec,
                    )
            except Exception as e:
                logger.warning(f"[extraction] SAS URL CU analysis failed for {filename}, using byte upload: {e}")

        return content_understanding_service.analyze(
            file=io.BytesIO(content),
            filename=filename,
            max_wait_sec=max_wait_sec,
        )
    ...
    @staticmethod
    def _parse_cu_fields(extracted) -> dict:
        """Pull topic + key phrases out of the CU field schema."""
        cu_fields: dict = {}
        if not extracted.fields:
            return cu_fields
        topic = extracted.fields.get("topic", {}).get("valueString")
        if topic:
            cu_fields["topic"] = topic
        key_phrases = extracted.fields.get("keyPhrases", {}).get("valueString")
        if key_phrases:
            cu_fields["key_phrases"] = [kp.strip() for kp in key_phrases.split(",") if kp.strip()]
        return cu_fields

    def _complete_extraction(self, container, message, file_id: str, filename: str, ext: str, extracted):
        """Persist CU output, mark the file chat-ready, and enqueue Stage 2 enrichment."""
        from src.api.modules.ingestion.service import ingestion_service

        cu_fields = self._parse_cu_fields(extracted)
        ...
        # Register the document so chat can find it immediately
        doc_data = {"id": file_id, "type": ext, "text": extracted.markdown, "metadata": base_metadata}
        if cu_fields.get("key_phrases"):
        ...
        # Store extracted text in blob (avoids the 64 KB queue message limit)
        text_blob_path = f"extracted/{file_id}/content.txt"
        try:
            container.get_blob_client(text_blob_path).upload_blob(
                extracted.markdown.encode("utf-8"), overwrite=True,
            )
        except Exception as e:
            logger.error(f"[extraction] Failed to store extracted text for {filename}: {e}")
            raise

        # Mark as extracted — available for chat even before chunking/indexing
        ingestion_service._update_file_status(file_id, "extracted")

        # Enqueue Stage 2: enrichment (reference only, no inline text)
        queue_service.enqueue(ENRICHMENT_QUEUE, {
            "file_id": file_id,
            "filename": filename,
            "ext": ext,
    ...
    def _handle_enrichment(self, message, payload: dict):
        ...
        if text_blob_path and not text:
            try:
                ...
                logger.info(
                    f"[enrichment] Extracted text blob missing for {filename} "
                    f"({text_blob_path}); dropping stale enrichment message"
                )
