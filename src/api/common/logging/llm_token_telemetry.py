...
class TokenUsageEmitter:
    ...
    connection_string:
        ...
        ``APPLICATIONINSIGHTS_CONNECTION_STRING`` env var is consulted.
    static_dimensions:
        Properties merged into every event (e.g. ``{"app": "CKM"}``).
    event_sink:
    ...
    pricing:
        ...
        usd_per_1k_output)}``. Attaches ``estimated_cost_usd`` to events.
    user_id_hasher:
        Optional callable ``str -> str`` applied to ``user_id`` values
        before emission. Use for PII / GDPR compliance.
    sample_rate:
        Fraction of high-cardinality events actually shipped, in [0.0, 1.0].
        The summary event always fires regardless of sample_rate.
    """
    ...
    def emit_summary(
        ...
        **dimensions: Any,
        ...
        """The summary event always fires (ignores ``sample_rate``)."""
    ...
    def emit_all(
        ...
        **dimensions: Any,
        ...
        """Convenience: emit summary, agent, and model events in one shot."""
        if not usage.has_any:
            return
